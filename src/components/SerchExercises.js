...
const SearchExercises = ({ setExercises, bodyPart, setBodyPart }) => {
  ...
  const handleSearch = async () => {
    if (search) {
      const exercisesData = await fetchData('https://exercisedb.p.rapidapi.com/exercises', exerciesoptions);
  ...
  return (
    <Stack alignItems="center" mt="30px" justifyContent="center" p="20px">
      <Typography fontWeight={700} sx={{ fontSize: { lg: '44px', xs: '30px' } }} mb="49px" textAlign="center">
        Search For Any Exercise <br /> You Want
      </Typography>
      ...
      <Box mb='50px' >
         <SizesExample  setBodyPart={setBodyPart}/>
      </Box>
      <UncontrolledExample/>
    </Stack>
  );
